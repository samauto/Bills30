...
<div class="search-index">
    ...
    foreach ($dataProvider->models as $model) {
        $doclat[$i] = $model->geoloc['latitude'];

        $docloc[$i] = $model->Provider_First_Line_Business_Practice_Location_Address
        ...
        if ($doclat[$i] != "") {
            $doclong[$i] = $model->geoloc['longitude'];
            $doclat[$i] = $model->geoloc['latitude'];
        }
        else {
            $doclat[$i] = "no geoloc lat";
            $doclong[$i] = "no geoloc long";
        }
        $i++;
    }


    //echo "<BR><BR>MIDDLE: ".count($docloc)."_count_<BR><BR>"; 
      #$GeoLocData[0] = $GeoLocModel->find(['GeoLoc'=>['address'=>$docloc[0]]])->one();
    for ($j = 0; $j < count($docloc); $j++)
    {
        echo "i1<>".$j."-".$doclat[$j]."<><BR>";
        echo "j".$j."<>".$doclat[$j]."<><BR>";
        ...
        //echo "AFTER: ".$doclat[$j]."_lat_";
        //echo $docloc[$j]."_doc_".$docNPI[$j]."_NPI_<BR><BR>";

        if ($doclat[$j] == "no geoloc lat") 
        {
            echo "saving";
        ...
        echo $longlat[0].' ';
        echo $longlat[1].'<BR>';
        $markers[$j] = new LatLng(['lat' => $doclat, 'lng' => $longlat]);
        //$markers[$j] = new LatLng(['lat' => $doclat[0], 'lng' => $doclong[0]]);
    }
   ...
   // echo $markers[0];
    ...
    $marker = new Marker([
        ...
        'popupContent' => $docloc[0]." ".$doclat.$doclong
        ]);
    ...
    for($m = 1; $m < count($markers); $m++)
    {
        ...
        $docmarker = new Marker([
            ...
            'popupContent' => $docloc[$m]." ".$doclat.$doclong
        ]);
